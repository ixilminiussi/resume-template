...
function createCustomItemEl(item) {
  ...
  el.innerHTML = tag === 'div' ? `<p>${text}</p>` : `<span>${text}</span>`;
  return el;
}
...
function openCreateForm(cfg, parent, onCreated, prefill = null, editId = null) {
  ...
  save.onclick = () => {
    ...
    el.setAttribute('data-custom', 'true');
    parent.appendChild(el);
    state.customItems.push({ id, parentSel: getCssSel(parent), textEn: data.titleEn || data.label || data.title || '', textFr: data.titleFr || data.label || '' });
